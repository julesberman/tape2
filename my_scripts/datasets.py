...
from aa_tokenizers import TAPETokenizer
...
class FluorescenceDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        token_ids = self.tokenizer.encode(item['primary'])
        input_mask = np.ones_like(token_ids)
        return item['primary'], token_ids, input_mask, float(item['log_fluorescence'][0])
...
class StabilityDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        token_ids = self.tokenizer.encode(item['primary'])
        input_mask = np.ones_like(token_ids)
        return item['primary'], token_ids, input_mask, float(item['stability_score'][0]), item['id']
...
class RemoteHomologyDataset(Dataset):
    ...
    def __init__(self,
        ...
        if split not in ('train', 'valid', 'test_fold_holdout',
                         'test_family_holdout', 'test_superfamily_holdout', 'test'):
            raise ValueError(f"Unrecognized split: {split}. Must be one of "
                             f"['train', 'valid', 'test_fold_holdout', "
                             f"'test_family_holdout', 'test_superfamily_holdout', 'test']")
        if isinstance(tokenizer, str):
            tokenizer = TAPETokenizer(vocab=tokenizer)
        self.tokenizer = tokenizer
    ...
    def __getitem__(self, index: int):
        ...
        token_ids = self.tokenizer.encode(item['primary'])
        input_mask = np.ones_like(token_ids)
        return item['primary'], token_ids, input_mask, item['fold_label'], item['id']
...
class SecondaryStructureDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        return item['primary'], token_ids, input_mask, labels, item['id']

...
def get_embeddings(seq_dict, cpu=False, model_dir=Path.cwd()/'ensemble'/'elmo'/'model', max_chars=15000, per_protein=False, verbose=False):

    # Pass in seq dict instead of read from file
    # seq_dict = dict() 
    # emb_dict = dict()
    ...
    for index, (identifier, sequence) in enumerate(seq_dict): # for all sequences in the set
        ...
        if length_counter > max_chars or len(sequence)>max_chars/2 or index==len(seq_dict)-1:
            ...
            if verbose: print('Embedding batch of {} sequeneces'.format(len(batch))  )
